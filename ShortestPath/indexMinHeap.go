...
func (h *indexMinHeap) extractMin() Weight {
	if h.count <= 0 {
		return nil
	}
...
func (h *indexMinHeap) change(i int, e Weight) {
	if !h.contain(i) {
		return
	}
	i++
...
func (h *indexMinHeap) insert(i int, data Weight) {
...
func (h *indexMinHeap) getMin() Weight {
	if h.count <= 0 {
		return nil
	}
